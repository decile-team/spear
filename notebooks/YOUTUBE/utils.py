...
def get_various_data(X, Y, X_feats, temp_len, validation_size = 100, test_size = 200, L_size = 100, U_size = None):
    ...
    index = np.random.RandomState(seed=42).permutation(index)
    X = X[index]
    Y = Y[index]
...
def get_test_U_V_data(X, X_feats, Y, temp_len, test_size = 100, validation_size = 100, U_size = None):
    if U_size == None:
        U_size = X.size - test_size - validation_size
    index = np.arange(X.size)
    index = np.random.RandomState(seed=42).permutation(index)
    X = X[index]
    Y = Y[index]
    ...
    return X_T,Y_T,R_T, X_U,R_U, X_feats_T, Y_U, X_feats_U, X_V, Y_V, X_feats_V