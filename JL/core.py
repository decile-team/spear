...
class JL:
	...
	Joint_Learning class:
		[Note: from here on, feature model and feature-based classification model are used interchangeably]
	...
	def fit(self, return_gm = False, path = None):
		...
		for run in range(0,self.n_runs):
			...
			for epoch in range(self.n_epochs):
				...
				if path != None:
					file.write("Run: {}\tEpoch: {}\tgm_valid_acc: {}\tlr_valid_acc: {}\n".format(run, epoch, gm_valid_acc, lr_valid_acc))
					if epoch % 10 == 0:
						file.write("Run: {}\tEpoch: {}\tgm_test_acc: {}\tlr_test_acc: {}\n".format(run, epoch, gm_acc, lr_acc))
		...
		if path != None:
			file.write("Training is done. gm_test_acc: {}\tlr_test_acc: {}\n".format(gm_acc, lr_acc))
			# print("===================================================")
